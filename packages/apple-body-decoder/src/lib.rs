...
mod tests {
    ...
    fn decode_json(bytes: &[u8]) -> String {
        serde_json::to_string(&decode_impl(bytes)).unwrap()
    }

    #[test]
    fn empty_input_is_a_typed_error() {
        assert!(decode_json(&[]).starts_with("{\"err\":"));
    }

    #[test]
    fn garbage_input_is_a_typed_error_not_a_panic() {
        let garbage: Vec<u8> = (0..1024u32).map(|i| (i * 7 % 251) as u8).collect();
        assert!(decode_json(&garbage).starts_with("{\"err\":"));
    }

    #[test]
    fn truncated_typedstream_header_is_contained() {
        // First bytes of a real typedstream header, cut short.
        let truncated = [0x04, 0x0b, b's', b't', b'r', b'e'];
        assert!(decode_json(&truncated).starts_with("{\"err\":"));
    }
}
